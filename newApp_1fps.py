...
# Process video frame by frame and save output with reduced FPS (1 FPS)
def process_video(input_video_path, output_video_path, model, target_fps=1):
    ...
    frame_interval = int(original_fps // target_fps)  # Interval to skip frames for 1 FPS
...
input_video = 'testVideoFull.mp4' # Input video path
output_video = 'output_with_garbage_boxes_1fps.mp4'  # Output video path

# Load the model and process the video
model = load_model(model_path)
process_video(input_video, output_video, model, target_fps=1)  # Set target FPS to 1
